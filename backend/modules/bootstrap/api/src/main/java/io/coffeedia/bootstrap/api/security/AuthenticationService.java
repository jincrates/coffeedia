 ...
 * 
 * 현재는 임시로 자체 JWT 구현을 사용합니다.
 * 추후 Keycloak 또는 다른 인증 서버와 연동 가능합니다.
 */
@Slf4j
@Service
...
public class AuthenticationService {
     ...
     * 로그인 처리
     * 
     * 현재는 데이터베이스에 저장된 사용자 정보로만 검증합니다.
     * 실제 비밀번호 검증은 추후 구현될 예정입니다.
     */
    public LoginResponse login(LoginRequest request) {
        // TODO: 실제로는 비밀번호 검증 로직 추가 필요
        // 현재는 임시로 사용자 존재 여부만 확인
        User user = userRepositoryPort.findByUsername(request.getUsername())
                .orElseThrow(() -> new UserNotFoundException("사용자를 찾을 수 없습니다: " + request.getUsername()));
    ...
    public LoginResponse refreshToken(String refreshToken) {
        ...
        // 새로운 토큰 생성
        String newAccessToken = jwtTokenProvider.createAccessToken(user.getUsername(), user.getRoles());
