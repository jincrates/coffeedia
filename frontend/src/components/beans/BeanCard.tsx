...
import {Calendar, Coffee, MapPin, Scale, Tag} from 'lucide-react';
import {
  formatRelativeDate,
  getBlendTypeKorean,
...
const BeanCard: React.FC<BeanCardProps> = ({
  ...
  // 수정 버튼: 로그인한 사용자만 표시 (소유권 체크는 커다란 더 없어서 일단 로그인만 체크)
  ...
  // 삭제 버튼: 로그인한 사용자만 표시 (소유권 체크는 커다란 더 없어서 일단 로그인만 체크)
      ...
      >
        <CardContent className="space-y-3">
          ...
          {bean.thumbnailUrl ? (
              ...
              <div
                  className="aspect-video rounded-lg bg-gradient-to-br from-coffee-100 to-coffee-200 flex items-center justify-center">
          ...
          <div>
            <div className="flex justify-between items-start mb-2">
              ...
              <span
                ...
                {bean.isDecaf && (
                    <span
                        className="inline-flex px-2 py-1 text-xs font-medium bg-orange-100 text-orange-800 rounded-full flex-shrink-0">
                  디카페인
                </span>
                )}
                {/* 액션 메뉴 */}
