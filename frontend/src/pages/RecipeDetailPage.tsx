...
import {ChefHat, Clock, ExternalLink, Lightbulb, ShoppingCart, Tag, Users} from 'lucide-react';
...
const RecipeDetailPage: React.FC = () => {
  ...
  const {requireOwnership, canPerformAction} = useAuth();
  ...
  return (
      <EntityDetailLayout
          ...
          onBack={handleBack}
          onEdit={recipe && canPerformAction('edit', {userId: recipe.userId}) ? handleEdit : undefined}
          onDelete={recipe && canPerformAction('delete', {userId: recipe.userId}) ? handleDelete : undefined}
          onShare={handleShare}
                    ...
                    {recipe.steps.map((step, index) => (
                        <div key={index} className="flex gap-4">
                          ...
                          <div className="flex-shrink-0">
                            <div
                                className="w-8 h-8 bg-coffee-600 text-white rounded-full flex items-center justify-center font-semibold text-sm">
                              {index + 1}
                            </div>
                          </div>
